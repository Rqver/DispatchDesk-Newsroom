...
async function fetchInformationFromRelease(link: string) {
    ...
    if (content) {
        reviewRelease("New Zealand Security Intelligence Service", "Media Release", content, link, mediaItems);
    }
}

async function onNewNZSISRelease(item: ParsedRssItem) {
...
export default createRssTask({
    name: "NZSIS Releases",
    description: "Listens to and logs media releases of the NZSIS.",
    url: "https://www.nzsis.govt.nz/news/rss",
