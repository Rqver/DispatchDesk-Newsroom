...
async function fetchInformationFromRelease(link: string) {
    ...
    $('.elemental.typography p').each((_idx, element) => {
    ...
    if (content) {
        reviewRelease("Pharmac", "Media Release", content, link, mediaItems);
    }
}

async function onPharmacRelease(item: ParsedRssItem) {
...
export default createRssTask({
    name: "Pharmac Media Releases",
    description: "Listens to and logs new Pharmac media releases.",
    url: "https://www.pharmac.govt.nz/news-and-resources/news/rss",
    frequencyMs: 60 * 1000,
    secondaryFrequency: {
        timePeriod: ["22:00", "06:00"],
        frequencyMs: 5 * 60 * 1000
    },
    onNewItem: onPharmacRelease
}) as Task;
